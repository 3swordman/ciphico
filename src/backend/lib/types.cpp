...
namespace backend::lib {
    ...
    object getitem_(std::deque<std::shared_ptr<ast::tree>>&& args) {
        ...
        if (temp == std::string::npos) {
            ...
            return args[0]->content.to_string().substr(std::stol(args[1]->content.data()->substr(0, temp)) - 1, std::stol(args[1]->content.data()->substr(temp + 1, args[1]->content.data()->size() - 2)));
        }
    }
